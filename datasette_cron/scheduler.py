...
class Scheduler:
    ...
    def register_handlers(self, plugin_name: str, handlers: dict[str, Callable[..., Any]]) -> None:
    ...
    async def _loop(self) -> None:
        logger.info("Scheduler loop started, handlers: %s", list(self._handler_registry.keys()))
        ...
        for task in due_tasks:
            ...
            if not handler_fn:
                logger.error(
                    ...
                    task["handler"], name, list(self._handler_registry.keys()),
                )
    ...
    async def _execute_task(self, task: dict, handler_fn: Callable[..., Any]) -> None:
        name = task["name"]
        config = json.loads(task["config"]) if isinstance(task["config"], str) else task["config"]
        max_attempts = task.get("retry_max", 0) + 1
        backoff_strategy = task.get("retry_backoff", "exponential")
        ...
        try:
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    duration_ms = int((time.monotonic() - start_time) * 1000)
                    await self.internal_db.record_run_error(run_id, "Cancelled", duration_ms)
                    raise
    ...
    async def _get_next_run_at(self, task: dict) -> datetime:
        now = _utcnow()
        sched = schedule_from_db(task["schedule_type"], task["schedule_config"], task["timezone"])
